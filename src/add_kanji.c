...
//		gtk_widget_modify_font (GTK_WIDGET (w->radical_entry), font_desc);
		...
		gtk_table_attach (GTK_TABLE (w->table1), w->meaning_label, 0, 1, 6, 7, GTK_EXPAND, GTK_SHRINK, 0, 0);
		...
		gtk_table_attach (GTK_TABLE (w->table1), w->meaning_entry, 1, 2, 6, 7, GTK_EXPAND, GTK_SHRINK, 0, 0);
...
//				g_slice_free (Widgets, w);
				...
				Kanji *tmp = kanji_create (kanji_str, radical_str, kun_str, on_str, meaning_str, jlpt_lvl, grade, stroke_cnt);
