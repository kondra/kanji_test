...
static void upd_entry (GtkWidget *cw, Widgets *w)
{
		...
		l5 = gtk_entry_get_text_length (GTK_ENTRY (w->meaning_entry));
...
//		GList *focus_chain = NULL;//temp solution
		...
		w.meaning_label = gtk_label_new ("Meaning:");
		w.jlpt_label = gtk_label_new ("JLPT Level:");
		w.grade_label = gtk_label_new ("School Grade:");
		w.radical_label = gtk_label_new ("Kanji Radical:");
		w.stroke_label = gtk_label_new ("Number of Kanji Strokes:");
		w.rst_label = gtk_label_new ("Number of Radical Strokes:");
...
//		gtk_widget_modify_font (GTK_WIDGET (w->radical_entry), font_desc);
		...
		w.table1 = gtk_table_new (8, 2, FALSE);
		...
		gtk_table_attach (GTK_TABLE (w.table1), w.rst_label, 0, 1, 4, 5, GTK_EXPAND, GTK_SHRINK, 0, 0);
		gtk_table_attach (GTK_TABLE (w.table1), w.on_label, 0, 1, 5, 6, GTK_EXPAND, GTK_SHRINK, 0, 0);
		gtk_table_attach (GTK_TABLE (w.table1), w.kun_label, 0, 1, 6, 7, GTK_EXPAND, GTK_SHRINK, 0, 0);
		gtk_table_attach (GTK_TABLE (w.table1), w.meaning_label, 0, 1, 7, 8, GTK_EXPAND, GTK_SHRINK, 0, 0);
		...
		gtk_table_attach (GTK_TABLE (w.table1), w.rst_spin, 1, 2, 4, 5, GTK_EXPAND, GTK_SHRINK, 0, 0);
		gtk_table_attach (GTK_TABLE (w.table1), w.on_entry, 1, 2, 5, 6, GTK_EXPAND, GTK_SHRINK, 0, 0);
		gtk_table_attach (GTK_TABLE (w.table1), w.kun_entry, 1, 2, 6, 7, GTK_EXPAND, GTK_SHRINK, 0, 0);
		gtk_table_attach (GTK_TABLE (w.table1), w.meaning_entry, 1, 2, 7, 8, GTK_EXPAND, GTK_SHRINK, 0, 0);
		...
		if (result == GTK_RESPONSE_OK)
		{
				...
				Kanji *tmp;
				tmp = kanji_create (kanji_str, radical_str, kun_str, on_str, meaning_str, jlpt_lvl, grade, stroke_cnt, rst_cnt);
